...
const defaultColumns = [
  {
    ...
    minWidth: 240,
    headerName: 'Name',
    renderCell: ({ row }: CellType) => <Typography>{row.permissionName}</Typography>
  },
  {
    flex: 0.35,
    minWidth: 280,
    ...
    renderCell: ({ row }: CellType) => {
      return (
        <div>
          {row.assignedTo &&
            row.assignedTo.map((assignee: string, index: number) => (
              <CustomChip
                size='small'
                key={index}
                skin='light'
    ...
    ...defaultColumns,
    {
      ...
      renderCell: ({ row }: CellType) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <IconButton onClick={() => handleEditPermission(row.permissionName)}>
