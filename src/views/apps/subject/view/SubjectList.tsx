...
const columns = [
            ...
            <AvatarGroup className='pull-up' sx={{ mr: 2 }}>
              {row.sections &&
                row.sections.map((section, index) => {
                  ...
                  const teachrF = section.teacher.f_name
                  const teachrL = section.teacher.l_name

                  return (
                    <Tooltip key={index} title={`Class of Teacher ${teachrF} ${teachrL}`}>
                      <StyledLink
                        href={`/apps/subject/view/${row.id}/${sectionId}/details`}
                        sx={{ height: 25, width: 25 }}
                      >
                        <CustomAvatar skin='light' color='primary' sx={{ height: 25, width: 25, fontSize: '1rem' }}>
                          {getInitials(sectionName)}
                        </CustomAvatar>
                      </StyledLink>
                    </Tooltip>
                  )
  ...
  },
  {
    ...
    minWidth: 150,
    headerName: 'Subject Classification',
    renderCell: ({ row }: CellType) => {
      return (
        <Typography variant='subtitle1' noWrap sx={{ textTransform: 'capitalize' }}>
          {row.classification.toLowerCase()} Subject
        </Typography>
      )
    }
  },
...
const SubjectList = ({ apiData, gradeId }: InferGetStaticPropsType<typeof getStaticProps>) => {
  // ** State
  ...
  }, [dispatch, gradeId, subjectTrack, setGradeLevel, subjectStrand, value])
