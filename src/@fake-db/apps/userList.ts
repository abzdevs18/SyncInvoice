...
const projectListData: ProjectListDataType[] = [
  {
    ...
    projectType: 'English',
    projectTitle: 'Social Media Task',
    img: '/images/icons/project-icons/social-label.png'
  }
]
...
mock.onGet('/apps/users/list').reply(config => {
  const { q = '', roles = null, status = null, currentPlan = null } = config.params ?? ''
  ...
  const filteredData = data.users.filter(
    user =>
      (user.username.toLowerCase().includes(queryLowered) ||
        ...
        user.roles.toLowerCase().includes(queryLowered) ||
