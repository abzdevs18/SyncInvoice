...
const columns = [
  {
    ...
    renderCell: ({ row }: CellType) => {
      const { l_name, f_name } = row

      return (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          {renderClient(row)}
          <Box sx={{ display: 'flex', alignItems: 'flex-start', flexDirection: 'column' }}>
            <StyledLink href='/apps/user/view/overview/'>{l_name}</StyledLink>
  ...
  },
  {
    ...
    renderCell: ({ row }: CellType) => {
      return (
        <Box sx={{ display: 'flex', alignItems: 'center', '& svg': { mr: 3, color: userRoleObj[row.roles].color } }}>
          <Icon icon={userRoleObj[row.roles].icon} fontSize={20} />
